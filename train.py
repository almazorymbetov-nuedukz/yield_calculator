...
from yield_calc.data import YieldConfig, FeatureEngineer, YieldDataset, build_molecular_cluster_dataset
from yield_calc.modules import YieldNet, YieldNetWithAttention, TransferLearningYieldNet
...
def train_model(args):
    ...
    if args.molecular_clusters:
        print(f"Building molecular-cluster features from: {args.molecular_clusters}")
        with open(args.molecular_clusters, "r", encoding="utf-8") as handle:
            cluster_payload = json.load(handle)
        molecular_data = build_molecular_cluster_dataset(cluster_payload, output_path=None)
        molecular_features = molecular_data["feature_matrix"]
        if molecular_features.shape[0] > 0:
            molecular_feature_names = molecular_data["feature_names"]
            print(f"Generated {molecular_features.shape[0]} molecular cluster samples with {molecular_features.shape[1]} features")
            molecular_feature_vector = molecular_features.mean(axis=0, keepdims=True)
            repeated_features = np.repeat(molecular_feature_vector, len(df_engineered), axis=0)
            molecular_frame = pd.DataFrame(repeated_features, columns=molecular_feature_names)
            df_engineered = pd.concat([df_engineered.reset_index(drop=True), molecular_frame], axis=1)

    # Prepare X and y
    if 'E' not in df_engineered.columns:
        raise ValueError("Training data must contain an 'E' target column")
    ...
    if args.model_type == "standard":
        ...
        model = TransferLearningYieldNet(
            input_dim=X.shape[1],
            hidden_dim=args.hidden_dim,
            pretrained_dim=max(8, min(X.shape[1], 8)),
            dropout=args.dropout,
        )
        print("Created: TransferLearningYieldNet (Molecular transfer-learning style)")
    else:
...
def main():
    ...
    parser.add_argument("--model_type", type=str, default="attention",
                       choices=["standard", "attention", "transfer"],
    ...
    parser.add_argument("--molecular_clusters", type=str, default=None,
                       help="Optional JSON file containing molecular cluster definitions")
